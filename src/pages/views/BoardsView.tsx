...
export default function BoardsView() {
  ...
  const { boards, isLoading, error, toggleStar, reload } = useBoardsQuery({
    q,
    filter,
    sort,
  });
  ...
  const ownerTeamsProvider = React.useCallback(async () => {
    ...
    return details
      .filter((team) =>
        team.members?.some((m) => m.user_id === myId && m.role === "owner")
      )
      .map((team) => ({ id: team.id, name: team.name }));
  }, [user?.id]);

  const handleDeleted = React.useCallback((id: number) => {
    void id; // <- utišava no-unused-vars, bez promene API-ja
    ...
    <div className="p-6">
      <div className="mb-2">
        ...
        <p className="text-sm text-zinc-500">
          All your project boards in one place.
        </p>
      </div>
