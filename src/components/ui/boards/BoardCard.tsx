...
type Props = {
  ...
  /** href koji otvara board kao VIEW (npr "/?v=board&id=123") */
  openHref?: string;
};

export default function BoardCard({ board, onToggleStar, onDeleted, onEdit, openHref }: Props) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl border bg-white shadow-sm transition-shadow border-zinc-200 hover:shadow-md hover:border-[rgb(34,211,238)]">
      {/* ⬇️ OTVARANJE TABLE PREKO VIEW-a */}
      <Link to={openHref ?? `/boards/${board.id}`} className="block">
        <div
          className="h-24 w-full"
          style={{
            background:
              board.cover ??
              `linear-gradient(135deg, ${TOKENS.accent} 0%, rgba(34,211,238,0.15) 100%)`,
          }}
        />
      </Link>
        ...
        <div ref={menuRef} className="absolute right-2 top-12">
          ...
          {openMenu && (
            <div className="absolute right-0 z-20 mt-2 w-40 overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-lg">
              <button
                className="block w-full px-3 py-2 text-left text-sm hover:bg-zinc-50 cursor-pointer"
                onClick={() => {
                  setOpenMenu(false);
                  onEdit?.(board);
                }}
              ...
              <button
                className="block w-full px-3 py-2 text-left text-sm text-rose-600 hover:bg-rose-50 cursor-pointer"
                onClick={() => {
                  setOpenMenu(false);
                  setConfirmOpen(true);
                }}
      ...
      <div className="p-3">
        <div className="mb-1 flex items-center justify-between">
          <Link to={openHref ?? `/boards/${board.id}`} className="line-clamp-1 font-semibold text-zinc-900 hover:underline">
            {board.title}
          </Link>
        </div>
        ...
        {board.tags && board.tags.length > 0 && (
          <div className="mb-2 flex flex-wrap gap-1.5">
            {board.tags.map((t, i) => (
              <span key={`${t}-${i}`} className="inline-flex items-center rounded-md border border-zinc-200 bg-zinc-50 px-2 py-0.5 text-[11px] text-zinc-700">
                {t}
              </span>
            ))}
          </div>
        ...
        <div className="fixed inset-0 z-[60] grid place-items-center bg-black/40 p-4" onClick={() => setConfirmOpen(false)}>
          <div className="w-full max-w-md rounded-2xl border border-zinc-200 bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="mb-5 text-sm text-zinc-700">
              This action will permanently delete the board and all its data. Team members linked to this board will lose access and the board
              cannot be recovered.
            </p>
